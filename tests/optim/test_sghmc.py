...
def test_sghmc() -> "None":
    ...
    try:
        _ = SGHMC(model.parameters(), lr=-1)
        _ = SGHMC(model.parameters(), num_burn_in_steps=-1)
        _ = SGHMC(model.parameters(), num_burn_in_steps=0.4)
        _ = SGHMC(model.parameters(), momentum_decay=-1)
        _ = SGHMC(model.parameters(), grad_noise=-1)
        _ = SGHMC(model.parameters(), epsilon=-1)
    except Exception:
        pass
    optimizer = SGHMC(model.parameters(), num_burn_in_steps=num_burn_in_steps)
